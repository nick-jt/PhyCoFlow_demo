#!/bin/bash
#SBATCH --job-name=classical_cyluonly
...
#SBATCH --mem=64G
#SBATCH --output=classical_cyl_uonly_%j.log
...
OUT=../Save_TrainedModel/cylinder2d_uonly/baseline_classical
mkdir -p "$OUT"

python -u baseline_classical_2d.py \
    --h5 "$H5" \
    --train-frames 1200 --fields Ux Uy p --cond-fields 0 \
    --methods constant kdtree idw gappy_pod --pod-rank 80 --idw-k 8 \
    --no-periodic --verify-percentile --tag uonly_pod80 \
    --out-dir "$OUT"
...
python -u baseline_classical_2d.py \
    ...
    --no-periodic --tag uonly_pod20 \
    --out-dir "$OUT"

echo "classical cyl uonly done"
