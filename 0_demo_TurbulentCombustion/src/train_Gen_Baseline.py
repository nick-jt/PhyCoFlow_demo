...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Unified trainer for generative baselines.")
    parser.add_argument(
        "--config",
        type=str,
...
def main() -> None:
    ...
    # -------------------------------------------------------------------------
    # Multi-stage routing and resume behavior.
    #
    # The adapter owns the baseline-specific model construction, but the top
    # level orchestrator owns "should we even allow this stage?" decisions.
    #
    # [EXTENSION POINT] If a future baseline adds a third stage, update the
    # config validator and this routing block, then implement the new adapter.
    # -------------------------------------------------------------------------
    ...
    if cfg["baseline_model"] == "latent_fm" and int(cfg["training_stage"]) == 2:
        ae_checkpoint = None if reload_ckpt is None else reload_ckpt.get("ae_checkpoint")
        if ae_checkpoint:
    ...
    # Save both the original input config backup and the resolved runtime config.
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        #
        # Each baseline keeps its own sampling / visualization logic, but every
        ...
        if epoch % save_every == 0:
            ...
            stage_sampling_cfg = stage_cfg.get("sampling", {})
            benchmark_steps = stage_sampling_cfg.get("benchmark_n_steps")
            if cfg["baseline_model"] == "latent_fm" and int(cfg["training_stage"]) == 1:
                benchmark_steps = [None]
            elif benchmark_steps is None:
                benchmark_steps = [stage_sampling_cfg.get("sampling_N")]

            with adapter.evaluation_weights(bundle):
                bundle.model.eval()
                for n_steps in benchmark_steps:
                    ...
                    step_label = "default" if n_steps is None else str(n_steps)
                    print(
                        f"[eval] epoch={epoch:04d} n_steps={step_label} "
