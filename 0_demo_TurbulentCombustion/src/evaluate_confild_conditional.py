...
def main() -> None:
    ...
    stage2_architecture = stage2["architecture"]
    train_indices = stage1["train_indices"]
    val_indices = stage1["val_indices"]
    ...
    decoder = Decoder(
        in_coord_features=3,
        in_latent_features=int(stage1_architecture["latent_dim"]),
    ...
    diffusion_model = create_model(
        image_size=int(stage2_architecture["model_image_size"]),
        num_channels=int(stage2_architecture["num_channels"]),
        num_res_blocks=int(stage2_architecture["num_res_blocks"]),
        num_heads=int(stage2_architecture["num_heads"]),
        num_head_channels=int(stage2_architecture["num_head_channels"]),
        attention_resolutions=stage2_architecture["attention_resolutions"],
        channel_mult=stage2_architecture["channel_mult"],
    ).to(device)
    ...
    target_row = (
        ...
        else int(stage2_architecture["window_length"]) // 2
    )
    if target_row >= int(stage2_architecture["window_length"]):
    ...
    for snapshot in args.snapshots:
        ...
        for member in range(args.ensemble_size):
            ...
            initial = torch.randn(
                1,
                1,
                int(stage2_architecture["window_length"]),
                int(stage1_architecture["latent_dim"]),
                device=device,
            )
