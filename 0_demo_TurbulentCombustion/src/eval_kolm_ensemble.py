...
def mean_std(per_snap: list[dict], path: list[str]) -> tuple[float, float]:
    vals = []
    for m in per_snap:
        d = m
        for k in path:
...
def main() -> None:
    ...
    def run_protocol(n_obs: int):
        ...
        for si, snap in enumerate(frames):
            ...
            if args.model in ("latent_fm", "sit"):
        ...
        cost = {
            ...
            "timing_note": ("dmfgen: total sample_ensemble wall-clock / K "
                            "(all K samples timed); baselines: k==0 draw "
                            "wall-clock. CUDA-synchronized; diagnostic "
                            "figures excluded." ),
        }
    ...
    if args.model == "dmfgen":
        ...
        print(f"[RESULT] {args.model} relL2={s['rel_l2_mean']:.5f} "
              f"crps={s['crps']:.5f} spread/err={s['spread_error_ratio']:.3f} "
