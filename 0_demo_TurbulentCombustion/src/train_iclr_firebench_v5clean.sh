#!/bin/bash
#SBATCH --job-name=iclr_fb_v5clean
...
DEMO_NUM=31
LOG_FILE="train_iclr_firebench_v5clean_${SLURM_JOB_ID}_DemoN${DEMO_NUM}.log"
echo "Job $SLURM_JOB_ID on $SLURM_NODELIST, python: $(which python)"
CUDA_VISIBLE_DEVICES=0 python train_pointcloud_ffm.py \
        --RELOAD \
        --config Save_config/config_iclr_firebench_v5clean.yaml \
