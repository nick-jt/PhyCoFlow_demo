...
\paragraph{Small-scale fidelity.} Energy spectra, structure functions and velocity-gradient PDFs of single samples (Figure~\ref{fig:spectra}, Appendix~\ref{app:extra}) test the premise behind avoiding an autoencoder. One estimator detail is load-bearing and we state it before the numbers: the cutouts are sub-blocks of a larger DNS and are therefore not periodic, so an unwindowed FFT spreads the edge discontinuity into a broadband floor. On our data the raw spectrum of the \emph{truth} falls only $2.4\times$ between $k{=}32$ and the Nyquist wavenumber, and a deliberately smooth Gaussian-process draw registers the same floor; with a variance-preserving Hann taper the truth falls $5.8\times10^{5}$ over that range. Band ratios computed without the taper are therefore ratios of two leakage floors and tend to unity for every method. All spectral numbers we report use the windowed estimator. \todo{Windowed re-analysis running; the numbers in this paragraph and Figure~\ref{fig:spectra} are being regenerated, and the earlier unwindowed dissipation-band comparison has been withdrawn.} Under the windowed estimator the inertial-range deficit is concentrated in the channels the sensors do not constrain, and is substantially larger than the unwindowed estimate suggested; Section~\ref{sec:results-jhu} reports the per-channel breakdown and the sampling-budget dependence.
