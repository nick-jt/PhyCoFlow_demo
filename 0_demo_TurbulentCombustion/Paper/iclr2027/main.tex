...
\end{center}
\caption{\textbf{Turbulence statistics of single posterior samples} on the held-out region (4 snapshots, NFE 4; ensemble means are deliberately not shown---averaging destroys small scales by construction). Left: shell-averaged energy spectra; the latent baseline carries more inertial-range energy, but truncates in the dissipation band (shaded), where \model{} tracks the DNS. Middle: longitudinal second-order structure functions. Right: velocity-gradient PDFs, where the latent baseline distorts the shoulders while both \model{} variants track the DNS shape.}
\label{fig:spectra}
\end{figure}

\paragraph{Small-scale fidelity.} Figure~\ref{fig:spectra} tests the premise behind avoiding an autoencoder and returns a more specific answer than the premise assumed. In band-integrated energy relative to the DNS, the latent baseline retains \emph{more} inertial-range energy than \model{} ($0.73$ vs.\ $0.56$ over $k\in[8,31]$) but truncates the dissipation range ($0.81$ vs.\ $0.97$ over $k\in[32,62]$): compression costs the smallest scales specifically, where the autoencoder's spectral floor was predicted to bite \citep{rafiq2026spectrally,glaws2020deep}, while the ambient model's deficit sits at intermediate scales. The spectral objective moves \model{} in the intended direction on both bands ($0.47\to0.56$, $0.88\to0.97$), and the velocity-gradient PDFs show the latent baseline distorting the shoulders of the distribution while both \model{} variants track the DNS shape. This is a crossover, not a uniform win: neither model reproduces the full spectrum from 1\% sensors, and what we can claim is that ambient generation preserves dissipation-range content that latent compression removes.
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.58\linewidth]{figures/scaling_cost.pdf}
\end{center}
\caption{\textbf{Measured cost versus output resolution} (H100, bf16, log--log; OOM markers are measured failures, not extrapolations), for training steps (top) and full-field posterior sampling (bottom). Grid methods (voxel diffusion, convolutional AE) scale cubically and fail at $320^3$ and $640^3$; point methods (\model{}, Senseiver) are flat in memory to $1024^3$ ($10^9$ points). Annotations give the fraction of the field supervised per training step: 100\% forced for grid methods, a fixed 39k-point budget for \model{} (2\% at $125^3$, 0.004\% at $1024^3$).}
\label{fig:scaling}
\end{figure}
