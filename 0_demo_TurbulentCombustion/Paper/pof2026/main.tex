...
Operator variants corrupt the assembled sensor sets by shared
...
CRPS at moderate relative $L_2$. \todo{cross-Re breakdown.}
...
relative $L_2$. \todo{CoNFiLD strict-2048 arm: its 50 per-snapshot files
aggregate to rel-$L_2$ 0.941 / CRPS 0.500 ($U_x$ 0.525, $U_z$ 0.559), not the
0.90 recorded earlier, and they carry no protocol stamp (no sensor count,
conditioned channels, seed or $K$), so the row is held out of this table until
its draw is confirmed canonical rather than admitted on the strength of a
filename.}}
\footnotesize
\begin{ruledtabular}
...
$n$. \todo{extend the paired analysis to the SiT-point, Senseiver, Gen4Turb
and DeepONet rows: their canonical per-snapshot payloads are not in the
current artifact set (SiT and Gen4Turb have none; Senseiver's uses an older
schema), so only the three rows above could be compared frame-by-frame.
Seed replicates (running) for the \dmfgen{} row.}
