...
Wilcoxon $p<10^{-14}$). \todo{DRAW PROVENANCE: this comparison pairs the frames but not
the sensors. The learned cylinder rows were scored on H100 and the classical
floors on GH200, and the 23{,}800-cell mesh draw is not reproducible across
those GPUs (frame 300: only 2 and 6 of 238 sensors per field coincide; grid,
Kolmogorov and JHU draws do reproduce). Each row is still an unbiased 1\%-draw
estimate, but the per-frame differences behind this interval mix method with
sensor layout. Fix: re-score the three mesh-native canonical rows on GH200 so
every mesh row shares one draw, re-run the paired test, then drop this note
(DELTA\_STATUS sec.26). The intro's ``identical seeded sensor draws''
(Sec.~\ref{sec:intro}) needs the same qualification.} Table~\ref{tab:cyl} reports the fleet: the closest
...
Sec.~\ref{sec:dp} reappears here as a spectral one. One confound is unresolved
at the time of writing: the samplers run at their own budgets rather than
matched (\dmfgen{} and latent FM at NFE~4, SiT at 50 steps, S3GM at 200), and a
coarse solver budget biases samples smooth.
...
appears in (b) only. Sampler budgets are each method's own and are not matched.}
\end{figure*}
