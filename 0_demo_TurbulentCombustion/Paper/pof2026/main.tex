...
two-dimensional turbulence (Geo-FNO 0.417) while being beaten on CRPS by three
of the four generative models, and latent flow matching, SiT and interpolation split the
...
observe only two of four channels. Multiphysics and unstructured-geometry
regimes are deferred to follow-up work; within this suite the cylinder's
body-fitted mesh already carries a surface-confined sensing variant
(Sec.~\ref{sec:surface}). Every method is evaluated under identical seeded sensor draws, identical
...
at $\mathrm{Re}=60,\,80,\,100,\,150,\,200,\,250$, against the empirical
$\mathrm{St}(60)\approx0.135$, $\mathrm{St}(100)\approx0.164$,
$\mathrm{St}(150)\approx0.183$, $\mathrm{St}(200)\approx0.196$ --- the
$3$--$5\%$ excess is the familiar two-dimensional over-prediction of the
shedding frequency, and the monotone trend and spacing are reproduced
(\texttt{src/check\_strouhal.py}). Mean drag lies between $1.40$ and $1.46$,
consistent with the 6\% blockage of the domain, and the lift-coefficient
amplitude increases monotonically with $\mathrm{Re}$ (0.16 to 0.88). The
dataset ships in two exports: the native
...
Wilcoxon $p<10^{-14}$). The best learned method here is Geo-FNO, which reads the grid export
and so, by protocol, receives its sensors on the grid rather than the mesh;
that pairing is over frames, not sensor positions. Against the mesh-native
methods, which share the classical floors' mesh, the comparison is paired in
both: re-scored on the floors' exact sensor draw, the three mesh rows move by
at most 0.006 (Senseiver 0.140, MLP-RBF 0.394, \dmfgen{} 0.221; the table
reports the original runs) and gappy POD's lead stays resolved against each
($-0.084$, $-0.339$, $-0.165$; all $p<10^{-14}$). Table~\ref{tab:cyl} reports the fleet: the closest
...
Between learned rows, which share one sensor draw and are fully paired,
\dmfgen{} places fourth: behind latent FM by 0.030 (95\% CI
$[0.014, 0.044]$) and behind Geo-FNO by 0.128 ($[0.123, 0.134]$, better on 0
of 50 snapshots).
...
by the same argument and its 128- and 360-tap entries are marked $\dagger$
because they are \emph{inferred} from the pool cap rather than re-measured ---
we spent the sampler budget on densities that could differ instead. The
identity is a measurement for three of the four rows and a construction for the
fourth, and the table distinguishes them. This is the
...
against all three ($-0.071$, CI $[-0.079,-0.063]$ versus \dmfgen{};
$p<10^{-14}$ in every case) and roughly seventeen times the training-seed spread.
...
\dmfgen{}, $-0.071$, CI $[-0.079,-0.063]$, $p<10^{-14}$), whereas the three
...
0.071, seventeen times the seed spread (Geo-FNO's own replicates were trained
at its original $4\times$ budget, so its seed spread is measured there), while \dmfgen{}--SiT (0.006) and
...
rather than resolved structure, and the nearest-neighbour interpolant reaches
...
samples; MLP-RBF, a ninth series past the validated eight-colour palette,
...
exclude, and per-call setup that a dedicated benchmark amortizes. The two kinds
are not interchangeable and we mark rather than mix
...
five rows fall monotonically --- S3GM steeply enough to pass every other
method, interpolation included, from 1965 sensors on --- Geo-FNO
\emph{degrades} past the canonical density (to 1.42 at 10\%, marked off-scale)
and latent FM past $\sim$2000 sensors, and Senseiver is nearly flat
throughout. Learned rows: canonical protocol, 50 held-out frames, $K$ as in
Table~\ref{tab:kolm}, sensor draws identical across methods at every density;
classical curves: same seeded sensor draws.}
\end{figure*}
...
is near the bottom of the fleet at 65 sensors (0.939, fifth of seven) and at
the top from 1965 sensors on: 0.135 there against SiT's 0.310, and 0.041 at
6554 --- five times better than interpolation and six times better than the
next learned row. Its curve is the steepest in the benchmark, $23\times$ over
the sweep against $3.4\times$ for \dmfgen{}, which is consistent with
posterior-sampling guidance whose likelihood term strengthens with the
measurement count until it overrides the prior; we report that mechanism as
consistent with the data, not established by it. The gain has two prices the
headline row does not show. It costs 22\,s per field against 0.19 for
\dmfgen{} (Table~\ref{tab:kolm}). And S3GM's stated spread does not shrink
with its error: spread--error rises from 1.07 at 1\% sensing to 2.56 at 10\%,
and 90\% coverage from 0.80 to 0.98, so the most accurate reconstruction in
the sweep is also its most over-dispersed. The density conclusion is therefore
not that learning stops paying at a few percent sensing; it is that
\emph{which} method pays is itself density-dependent. The leader of the
single-density ranking in Table~\ref{tab:kolm}, Geo-FNO, is last of seven at
10\% sensing --- worse than predicting the training mean --- and the 10\% leader is a row the 1\% table ranks fourth.

The fleet also does not share a slope, and the exceptions are diagnostic rather
than noisy. Five rows fall monotonically over the full $100\times$ range
(S3GM $0.939\!\to\!0.041$, \dmfgen{} $0.887\!\to\!0.258$, SiT
$0.940\!\to\!0.261$, MLP-RBF $0.936\!\to\!0.578$, Senseiver
$0.942\!\to\!0.907$). Two do not: Geo-FNO is best at the
canonical density and degrades from there --- 0.417 at 655 sensors, 0.463 at
1965 and \emph{1.42} at 6554, worse than predicting the training mean --- and
latent FM turns the same corner later and more gently ($0.495\!\to\!0.557$
past 1965 sensors). Both are
...
would make it more than three times as bad. Senseiver's near-flatness is the same lesson from
...
freedom and dissipation-range fidelity, measured here, and operator robustness,
which the deferred multiphysics regime is designed to measure
...
\todo{PROVENANCE: the 3D pair above (0.14 shuffled / 0.56 honest) comes from
archived pre-fix evaluations that are not on the current host; per the text
they were single-snapshot at a different solver setting. The honest 0.56 must
therefore not be read against Table~\ref{tab:jhu}, whose latent-FM row is 0.469
over 50 snapshots (0.56 is that row's \emph{coverage}); state the archived
setting beside the pair. The ``factors of 2--4'' range has no tabulated source
in the artifact set.}
...
inverts on Kolmogorov (Sec.~\ref{sec:kolmogorov}) --- with geometry-freedom and
...
spanning 2D laminar to 3D chaotic flow,
