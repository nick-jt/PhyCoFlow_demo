...
improves them by under 4\% in both 2D and 3D, so split leakage is a correctness
requirement rather than the source of the gap. Datasets,
...
the strongest leak a split can produce --- improves them by less than 4\%
relative $L_2$ in both 2D and 3D (Sec.~\ref{sec:leakage}). At benchmark data
scales these models learn the sensing operator rather than a lookup table, so
...
(IDW 0.589 against 0.589), so the training frames are not easier: the
largest in-sample gain is 3.3\%.
...
more than three percent from having seen its evaluation targets during
training. At ${\sim}6.5$M parameters and $10^5$--$10^6$ points per frame
