...
experimental and operational fluid mechanics, now addressed by a growing
machine-learning literature of deterministic operator learners and conditional
generative models. The field has no shared evaluation: published methods use
incompatible, often temporally leaky splits, ensembles are drawn but not scored
with proper probabilistic metrics, and classical baselines are rarely reported.
We present \benchname{}, a benchmark spanning three flow regimes of increasing
dimensionality and dynamical complexity --- laminar cylinder vortex shedding
across Reynolds numbers, two-dimensional Kolmogorov turbulence, and
three-dimensional isotropic turbulence (JHTDB) --- with leakage-audited splits,
frozen sensor manifests, per-channel accuracy, proper scoring rules (CRPS,
coverage, spread--error), physics metrics, and matched cost accounting. Across
eleven learned methods and four classical floors, method ranking is
regime-dependent and no method wins twice: gappy POD with 20 modes wins the
laminar cylinder outright (0.056 relative $L_2$, including the never-observed
pressure), a deterministic operator learner leads two-dimensional turbulence
(Geo-FNO, 0.417) yet is beaten on CRPS by three of four generative models, and
latent flow matching, a point-token transformer, and plain interpolation split
the channels of 3D turbulence. No method recovers the unobserved channels of 3D
turbulence; nearly every ensemble is under-dispersed, and split-conformal
recalibration restores stated coverage. Benchmark scores are systematically
worse than literature numbers on the same data, and controlled ablations
attribute the gap to evaluation protocol rather than model capability: the same
checkpoint improves by 25\% when scattered sensors are rearranged onto a uniform
lattice, whereas re-scoring frozen checkpoints on their own training frames
changes their observed-channel error by at most 5\% in both 2D and 3D, so split
leakage is a correctness requirement rather than the source of the gap. Datasets,
splits, sensor manifests, configurations, and the evaluation driver are released
as a versioned artifact. \todo{artifact DOI}
\end{abstract}
...
for scientific machine learning --- PDEBench,\cite{takamoto2022pdebench} The
Well,\cite{ohana2024well} CFDBench,\cite{luo2023cfdbench}
AirfRANS,\cite{bonnet2022airfrans} and their successors --- pose
forward-surrogate or uniform-downsampling
...
autocorrelated in time (frame-to-frame correlation 1.00 at unit separation on
our 3D data), so the standard random-in-time split makes every test frame a
near-duplicate of a training frame. We expected this to dominate and measured
it directly: re-scoring frozen checkpoints on their own training frames ---
the strongest leak a split can produce --- changes their observed-channel
error by at most 5\% in both 2D and 3D, and the one larger gain sits on an
unobserved 3D channel that no split makes recoverable
(Sec.~\ref{sec:leakage}). At benchmark data scales these models learn the
sensing operator rather than a lookup table, so
leakage control is a correctness requirement, not the explanation for the gap
to the literature. Second, \emph{ensembles are drawn but not scored}:
...
proper-orthogonal-decomposition floors are rarely reported, and we find that
inverse-distance weighting matches or beats all but one deep model's
observed-channel error on 3D turbulence at 1\% sensor density, and that gappy
POD wins the laminar regime outright. A consequence worth stating
...
\benchname{} is our response: a benchmark built to answer the question a
practitioner actually has --- \emph{which method family should I use, in which
flow regime, at what sensor budget, with what uncertainty and at what cost?}
It spans three regimes forming a ladder in dimensionality and dynamical
complexity (Sec.~\ref{sec:datasets}): laminar cylinder vortex shedding, where
classical linear methods should be near-optimal; two-dimensional Kolmogorov
turbulence, the standard testbed of the 2D generative
literature;\cite{shu2023physics,li2024s3gm} and three-dimensional isotropic
turbulence under a spatially disjoint protocol in which sensors observe only
two of four channels. Every method is evaluated under identical seeded sensor
draws, leakage-controlled splits with a strict tune/test separation,
per-channel accuracy, proper scores and calibration diagnostics, windowed
spectral metrics, and measured training and inference cost.
...
fidelity-audited fleet of eleven learned methods and four classical floors
spanning deterministic operator learners and latent and ambient generative
...
The three regimes form a ladder in both spatial dimension and dynamical
complexity --- 2D laminar and periodic, 2D chaotic, 3D chaotic --- so that
each step isolates one source of difficulty while holding the task fixed
(Table~\ref{tab:regimes}); preprocessing details and download/regeneration
recipes ship with the artifact.

\begin{table*}[tbp]
\caption{\label{tab:regimes}The three regimes of \benchname{}. Each step of
the ladder adds one source of difficulty; the split column names the
leakage-controlled protocol (Sec.~\ref{sec:splits}).}
\footnotesize
\begin{ruledtabular}
\begin{tabular}{llllll}
Regime & Dataset & Points & Channels & Split & Axis isolated \\
\hline
2D laminar periodic & Cylinder wake (new) & $2.4{\times}10^{4}$ & $u,v,p$ & cross-Re & low intrinsic dim. \\
2D chaotic & Kolmogorov $256^2$\cite{shu2023physics} & $6.6{\times}10^{4}$ & $\omega$ & cross-traj. & 2D multiscale \\
3D chaotic & JHTDB isotropic\cite{li2008public} & $1.95{\times}10^{6}$ & $U_x,U_y,U_z,p$ & spatial & unobs.\ channels \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
a 12-block O-grid of 23{,}800 cells with boundary-layer grading, at
$\mathrm{Re}\in\{60,80,100,150,200,250\}$ ($D=U_\infty=1$). Each case runs a
transient phase to $t=120$ and a sampling phase to $t=270$ written every
$0.5$ time units: 300 snapshots, roughly 25 shedding cycles at
$\mathrm{Re}=100$. The physics-level validation is the Strouhal number from
the lift spectrum, $\mathrm{St}=0.140,\,0.160,\,0.173,\,0.187,\,0.200,\,0.207$
at $\mathrm{Re}=60$--$250$ against the empirical $0.135$, $0.164$, $0.183$
and $0.196$ at $\mathrm{Re}=60$, 100, 150 and
200;\cite{williamson1996vortex,norberg2003fluctuating} the $3$--$5\%$ excess
is the familiar two-dimensional over-prediction, and the trend and spacing are
reproduced. Mean drag lies between $1.40$ and $1.46$, consistent with the 6\%
blockage, and lift amplitude grows monotonically with $\mathrm{Re}$. The
dataset ships as the native cell-center point cloud, with the wall-adjacent
cell ring indexed for the surface-tap variant, and as a $400\times200$
uniform-grid interpolation over $x\in[-3,17]$, $y\in[-5,5]$ with a body mask
for grid-locked methods. Cross-Re protocol: train on
$\mathrm{Re}\in\{60,100,150,200\}$, evaluate on the held-out
$\mathrm{Re}\in\{80,250\}$ (one interpolative, one mildly extrapolative), with
temporally blocked splits inside the training cases. An observe-$u$-only
variant probes cross-channel inference ($v$, $p$ unobserved) in a regime where
the shedding phase makes the unobserved channels largely determined by the
observed one.
...
generative-reconstruction literature: Kolmogorov flow on the doubly periodic
box $[0,2\pi)^2$ at Reynolds number 1000 with wavenumber-4 forcing, 40
independent trajectories of 320 frames at $256^2$. We stride time by 4 (80 frames per trajectory, 3{,}200
...
(forced isotropic turbulence at $Re_\lambda\approx418$; each cutout spans
${\approx}0.56$ integral length scales at a grid spacing of ${\approx}2.1$
Kolmogorov scales), 50 snapshots each at a stride of 100 database steps,
fields $(U_x,U_y,U_z,p)$, per-field standardized. Training uses cutouts 1--3; all
...
separation 100; random-in-time splits therefore risk measuring near-duplicate
interpolation (Sec.~\ref{sec:leakage} measures how much they actually do). The suite
...
observations. The cross-checks the artifact runs automatically were added
after two construction-time incidents (a baseline's evaluation path collapsing
a spatial axis, and a one-frame boundary leak from integer truncation in the
split arithmetic), both caught before any number was reported.

\subsection{Sensor protocols}\label{sec:sensors}
...
evaluation. Sensor layout is not incidental to difficulty: at fixed count,
rearranging scattered sensors onto a uniform lattice changes reconstruction
error by tens of percent (Sec.~\ref{sec:leakage}), so the layout is part of
the protocol specification rather than an implementation detail. Two layout
variants accompany the random-scatter default --- a uniform lattice at matched
count, and sensors confined to the cylinder wall ring (Sec.~\ref{sec:surface})
--- and channel subsets are specified per regime (Sec.~\ref{sec:datasets}).
...
observed quantities (implemented, but not exercised in the reported results:
the operator study runs on Kolmogorov, which observes a single channel;
Table~\ref{tab:operator}). Corrupted runs carry their own protocol stamp so they can
never be aggregated with clean ones. Distinguishing per-frame, per-trajectory, and globally
fixed sensor masks matters for comparability across papers, because the three
make materially different problems out of the same nominal sensor budget.
\benchname{} uses per-frame draws with frozen evaluation manifests, and says
so explicitly, precisely so that a disagreement with a paper using another
convention can be recognized as a protocol difference rather than a result.
...
\paragraph{Statistical resolution.} Every learned row of a regime is scored on
the same frames under the same seeded draws, so the per-snapshot difference
between two methods is the unit of analysis. We report the mean paired
difference with a 95\% percentile-bootstrap interval (20{,}000 resamples) and
call a gap \emph{resolved} only when the interval excludes zero and a Wilcoxon
signed-rank test agrees at $p<0.05$; methods whose gaps are not resolved are
reported as a tie group rather than ranked. On the two 2D regimes every method
was additionally retrained under two further seeds, so a gap can be compared
with the training-seed spread as well as with the frame-resampling interval.
...
is constructed against our own biases. (i) \emph{Upstream
...
or labeled before reporting. (ii) \emph{Parameter matching}: learned methods
...
which budgets exceed which. Auditing configured against as-run step counts for
all fourteen 2D rows found thirteen within 4\% of ${\sim}50$k steps and one
budget error --- Kolmogorov Geo-FNO at 200k steps, because its batch size was
a quarter of the reference and the budget had been fixed in epochs; the
matched rerun replaces it (Sec.~\ref{sec:kolmogorov}). (v)
\emph{Tuning-budget disclosure}: the
...
every tuned variant is a separately labeled arm. (vi) \emph{Selection
...
Fig.~\ref{fig:capability} expands it to the per-method deployment axes that
...
inverts that profile. Kolmogorov and the 3D cutouts are periodic Cartesian
grids on which every family runs natively, so the interface filter is
exercised here only by the cylinder's body-fitted mesh, at its mildest.
Per-method configurations, adaptations, and audit notes ship with the
artifact.
...
or adapted, open = no). Behavior under corrupted operators is a graded
measurement rather than a capability and is reported in
Table~\ref{tab:operator}; ``dissipation-preserving'' records the windowed dissipation-band ratios
(Sec.~\ref{sec:spectra}); ``memory flat vs
resolution'' and ``sub-second inference'' record the measured scaling study
(Sec.~\ref{sec:cost}); ``recalibrable'' records whether the post-hoc wrapper
of Sec.~\ref{sec:calibration} applies. Axes are marked ``no'' only when the
failure is structural.}
\end{figure*}

\begin{table}[tbp]
\caption{\label{tab:capability}Capability matrix: which method families can
enter which regimes. RS = requires resampling to a grid, lossy near the
body-fitted boundary of the cylinder mesh, the only regime here that
exercises the distinction; each cylinder cell records the export the
corresponding run actually opened (Senseiver, MLP-RBF and \dmfgen{} read the
23{,}800-cell mesh; Geo-FNO, latent FM, S3GM and SiT the $400\times200$ grid
export). $^{\S}$SiT is point-tokenized in 3D but patch-tokenized in both 2D
runs: the family can enter grid-free, this configuration did not. Rows are
families rather than methods because the claim is about interfaces.}
\begin{ruledtabular}
\begin{tabular}{lccc}
Family & Cyl. & Kolm. & JHU \\
\hline
...
sensor randomization as the amortized generative methods. Geo-FNO
enters the two 2D regimes as the grid-based deterministic learner: a Fourier
neural operator\cite{li2021fourier,li2023geofno} that reads the sensors
rasterized onto the grid as a value channel and a binary mask channel per
field and returns the full field in one forward pass, so on the cylinder it
consumes the uniform-grid export. MLP-RBF is a point-native deterministic
control for the same regimes: each query gathers the sensor tokens through a
softmax radial-basis kernel over sensor distance plus a mean-pooled global
summary and is trained by direct regression, which isolates the point-native
conditioning path from the generative formulation. FNO3D places a
...
(``DeepONet++'', multiresolution binned pooling) is reported as a labeled
...
than the mesh (Table~\ref{tab:capability}). Its single samples carry
chunk-incoherent speckle that ensemble averaging hides, one reason
\benchname{} reports member-level metrics.
...
Table~\ref{tab:summary} collects the leader of each regime by method family;
the three subsections give the evidence and the paired resolution behind each
entry.

% Hand-assembled from the generated bodies tab_cyl_body.tex, tab_kolm_body.tex
% and the tab:jhu rows; re-check against them whenever those regenerate.
\begin{table*}[tbp]
\caption{\label{tab:summary}Who wins where. Leaders per regime at the headline
sensor density (1\%), by method family, as aggregate relative $L_2$ of the
ensemble mean; per-channel values, CRPS and cost are in Tables~\ref{tab:cyl},
\ref{tab:kolm} and~\ref{tab:jhu}. ``Tie'' marks a generative leader whose gap
to the rows behind it is not resolved by paired intervals
(Sec.~\ref{sec:metrics}). On 3D turbulence the aggregate averages two observed
and two unobserved channels; on the observed channels alone SiT-point leads
(0.143 on $U_x$) with interpolation second (0.174). The last column asks
whether any method recovers a channel the sensors never observe.}
\footnotesize
\begin{ruledtabular}
\begin{tabular}{llllll}
Regime & Best classical & Best deterministic & Best generative & Lowest CRPS & Unobserved channel \\
\hline
Cylinder wake & \textbf{Gappy POD 0.056} & Geo-FNO 0.093 & Latent FM 0.192 & Gappy POD 0.028 & recovered ($p$ 0.051) \\
Kolmogorov & IDW 0.531 & \textbf{Geo-FNO 0.417} & \dmfgen{} 0.487 (tie) & S3GM 0.231 & one channel only \\
3D turbulence & IDW 0.589 & Senseiver 0.699 & \textbf{Latent FM 0.469} & Latent FM 0.244 & not recovered (best 0.60) \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
The dataset was generated for this benchmark and validated against the
Strouhal and drag literature before any model saw it
(Sec.~\ref{sec:datasets}). Sensors observe
...
It does not, and the gap is resolved rather than marginal
(Table~\ref{tab:cyl}): on the same 50 frames, rank-20 gappy POD beats the best
learned method, Geo-FNO at 0.093, by $0.037$ relative $L_2$ (95\% CI
$[-0.042,-0.032]$, $p<10^{-14}$), and the generative models trail further
(latent FM 0.192, \dmfgen{} 0.221, SiT 0.328). Geo-FNO reads the grid export
and receives its sensors on the grid rather than the mesh, so that pairing is
over frames only; against the mesh-native rows, re-scored on the floors' exact
sensor draw (the table reports the original runs, which differ by at most
0.006), POD's lead is resolved with the same confidence (Senseiver $-0.084$,
\dmfgen{} $-0.165$, MLP-RBF $-0.339$; all $p<10^{-14}$). Among the learned
rows \dmfgen{} places fourth, behind latent FM by 0.030 ($[0.014, 0.044]$) and
behind Geo-FNO by 0.128 ($[0.123, 0.134]$, better on none of the 50
snapshots).
...
CRPS at moderate relative $L_2$. \dmfgen{} sits on the other side
of unity (spread--error 0.57, coverage$_{90}$ 0.51), so the two generative
routes miss calibration in opposite directions on the same flow --- an
argument for the post-hoc recalibration of Sec.~\ref{sec:calibration} rather
than for either architecture.

\emph{Which held-out Reynolds number?} The training set is
$Re\in\{60,100,150,200\}$, so $Re=80$ is an interpolation in Reynolds number
and $Re=250$ an extrapolation past the training maximum, and averaging the
two hides the more informative question. Splitting the same 50 frames along
that line (Table~\ref{tab:crossre}) gives three results. The ranking does not
change: gappy POD leads at both Reynolds numbers, Geo-FNO is second at both,
Senseiver third. Extrapolating costs every working method between
$1.26\times$ and $1.54\times$, and the cost is largest for the most accurate
rows --- gappy POD $1.54\times$ (0.044 to 0.068), Geo-FNO $1.50\times$ --- and
smallest for rows already near the training-mean floor (interpolation
$1.11\times$): the methods that exploit the training family's structure
hardest lose most when asked to leave it, which is the counterweight to the
headline that classical methods win this regime. And the never-observed
pressure is the most Reynolds-sensitive part of the reconstruction: six of the
seven rows that recover $p$ at all degrade faster on $p$ than on their
aggregate (gappy POD $1.73\times$ against $1.54\times$, Senseiver $1.66\times$
against $1.26\times$, \dmfgen{} $1.51\times$ against $1.30\times$; SiT is the
exception), as expected for a channel reconstructed entirely from
cross-channel structure learned at the training Reynolds numbers. This also
explains the scalar-recalibration failure of Sec.~\ref{sec:calibration}: a
single multiplier fitted across an interpolation and an extrapolation is
fitting two error scales at once.
...
\hline
\input{tab_crossre_body}
\end{tabular}
\end{ruledtabular}
\end{table}

\begin{figure*}[tbp]
\includegraphics[width=\textwidth]{figures/gallery_cylinder_allbaselines_paper.pdf}
\caption{\label{fig:qual-cyl}Cylinder wake, held-out $\mathrm{Re}=250$ frame (val 300).
Rows are the three reconstructed channels: $u_x$ and $u_y$ observed at 238
sensors per field (1\%; circles on the truth panels) and $p$ never observed.
Columns are the truth and then one baseline each. Mesh-native methods
(\dmfgen{}, Senseiver, MLP-RBF, IDW, gappy POD) share one sensor draw;
grid-native methods (SiT, latent FM, S3GM, Geo-FNO) receive the same count on
the grid export, exactly as in Table~\ref{tab:cyl}. Generative columns show one
posterior sample, not the ensemble mean; the number in each panel is that
field's single-sample relative $L_2$ in standardized units, so it sits above the
table's ensemble-mean error. The $p$ row is the point: gappy POD infers the
unobserved channel through the basis's cross-channel correlations, S3GM's
sampler diverges on this dataset, and interpolation returns the training mean
by construction.}
\end{figure*}
...
while the three mesh-native methods keep improving to 360. S3GM is grid-locked
by the same argument; its 128- and 360-tap entries are marked $\dagger$
because they are inferred from the pool cap rather than re-measured. This is
the capability matrix's ``requires resampling'' column measured rather than
asserted: the interface a method demands is a scientific property, not an
...
third at 0.245, behind both deterministic rows, and does not lead on CRPS
either. We record the failed prediction.
...
$\mathrm{Re}=250$ frame (128 taps per field, black dots on the body);
everything off the body is inferred. Per-panel numbers are single-sample
relative $L_2$ per field. Read against Fig.~\ref{fig:qual-cyl}, the identical
frame sensed in the volume: gappy POD, which wins there, is beaten several-fold
here, and interpolation is worse than the training mean.}
\end{figure}
...
sweep (Sec.~\ref{sec:density}). Three regime findings stand out.
...
coverage cell is empty, their CRPS equals the MAE exactly, and their mean and
single-sample columns coincide. Inference cost per field on one H100
(Geo-FNO's budget-matched run: one GH200). Rows are generated from the
canonical JSONs by \texttt{src/figs\_pof/make\_2d\_tables.py}.}
\footnotesize
\begin{ruledtabular}
\begin{tabular}{lcccccc}
 & \multicolumn{2}{c}{Rel.\ $L_2$} & & & & \\
Method & mean & sample & CRPS & Cov.$_{90}$ & s/field & GB \\
\hline
\input{tab_kolm_body}
\end{tabular}
\end{ruledtabular}
...
outright at 0.417 --- 14.5\% better than the best generative model --- while
latent FM, the 3D aggregate winner, trails even the interpolation floor here
(0.599 against 0.531) at the matched ${\sim}50$k-step budget, and
\dmfgen{} beats it on both relative $L_2$ and CRPS on all 50 paired
snapshots. The remaining
...
$L_2$ is beaten on CRPS by three of the four generative models (0.287 against S3GM
0.231, SiT 0.234, \dmfgen{} 0.235; only latent FM, at 0.310, is worse), and the
split is resolved on paired snapshots: Geo-FNO trails \dmfgen{} on CRPS by 0.052
(95\% CI $[0.048, 0.057]$, all 50), while \dmfgen{} and SiT are indistinguishable
on it ($0.001$, $[-0.002, 0.004]$). This is the distortion--perception tradeoff of
...
The winning row initially received four times the fleet's training budget:
the Kolmogorov budget was set in epochs at batch size 128, and Geo-FNO
inherited the epoch count at batch size 32, hence 200k optimizer steps against
the ${\sim}50$k of every other row (Sec.~\ref{sec:fairness}). Retrained at
exactly 50k steps with the configuration otherwise untouched, \emph{Geo-FNO
still wins}: 0.417 against \dmfgen{}'s 0.487, SiT's 0.494 and S3GM's 0.498.
The margin narrows from 21\% to 14.5\%; the four-fold budget was worth 0.032
relative $L_2$ to this architecture, a real effect and smaller than the gap it
was suspected of manufacturing. The matched run is the row and the
over-trained one an ablation, because a leaderboard should answer what a
method achieves at the benchmark's budget.

The paired analysis of Sec.~\ref{sec:metrics} resolves Geo-FNO's lead against
the whole fleet ($-0.071$ versus \dmfgen{}, CI $[-0.079,-0.063]$,
$p<10^{-14}$), whereas the three generative models behind it are not separable
from each other: \dmfgen{} versus SiT gives $-0.006$ with $p=0.16$, SiT versus
S3GM $-0.004$ with $p=0.32$, and only the \dmfgen{}--S3GM pair separates
($-0.010$, $p=0.008$), the expected intransitivity when three methods sit
inside a 0.011 band. We report them as a tie group. Retraining every 2D method
under two further seeds (7 and 1337) gives the same answer from the other
side: the pooled training-seed standard deviation is 0.0041 relative $L_2$ on
Kolmogorov and 0.0081 on the cylinder (S3GM is the most seed-sensitive row in
both regimes, 0.0082 and 0.0163, consistent with its sampler fragility; SiT
the least on Kolmogorov, 0.0006). Geo-FNO's lead is seventeen seed standard
deviations (its replicates were trained at the original $4\times$ budget, so
its spread is measured there), while \dmfgen{}--SiT and SiT--S3GM are one to
one-and-a-half apart and cannot be called ordered; on the cylinder every
adjacent gap exceeds the pooled spread by at least $3.7\times$, the tightest
being latent FM against \dmfgen{} at 0.030. Frame resampling and training-run
resampling therefore agree on which comparisons this benchmark resolves.

The measurement operators of Sec.~\ref{sec:sensors} put the same ordering
under corrupted observations (Table~\ref{tab:operator}). Gaussian noise at
$\sigma = 0.1$ is invisible at this precision: no row moves by more than
0.007. At $\sigma = 0.3$ the four leading rows lose 6--12\% (Geo-FNO
$0.417\to0.457$, \dmfgen{} $0.487\to0.525$, SiT $0.494\to0.555$, S3GM
$0.498\to0.526$). A contiguous slab removing a quarter of the sensors costs
those rows far more, 31--44\%, because it leaves a region with no nearby data
rather than thinning the data everywhere. Geo-FNO keeps its lead under every
operator (0.554 occluded, against 0.651 for the next row). Below it the clean
tie group reshuffles: SiT degrades most ($1.44\times$) and ends level with
latent FM (0.709 against 0.695), which it led by 0.105 on clean data, while
S3GM (0.651) and \dmfgen{} (0.660) stay 0.009 apart --- about two clean-fleet
seed standard deviations --- and we do not rank them. The rows that barely
notice any operator are the ones with the highest clean error: Senseiver moves
from 0.913 to 0.920 under occlusion, and latent FM and MLP-RBF lose 16--19\%.
Read alone, the last column of Table~\ref{tab:operator} would rank the least
accurate model as the most robust; insensitivity to losing sensors is only a
virtue in a model that was using them. Channel dropout is not in the table:
Kolmogorov observes a single channel, so removing it would leave nothing to
condition on.

\begin{table}[tbp]
\caption{\label{tab:operator}Measurement operators on Kolmogorov at 1\%
sensing (655 sensors): ensemble-mean relative $L_2$ over the 50 canonical test
frames ($K{=}8$ for generative rows, one forward pass for deterministic ones).
Every method receives bit-identical corrupted observations
(Sec.~\ref{sec:sensors}); $\sigma$ is Gaussian noise in standardized units, and
the slab removes one contiguous region holding 25\% of the sensors. Last
column: occluded over clean. Geo-FNO is the budget-matched run.}
\begin{ruledtabular}
\begin{tabular}{lccccc}
Method & clean & $\sigma{=}0.1$ & $\sigma{=}0.3$ & 25\% slab & slab/clean \\
\hline
\input{tab_operator_body.tex}
\end{tabular}
\end{ruledtabular}
\end{table}

Spectrally the fleet separates differently again (Fig.~\ref{fig:spectra-kolm}).
Measured against the truth spectrum of the same frame over a small-scale band
($k = 32$--$128$), the samplers that inject texture overshoot --- latent FM
carries 1.17 of the true energy, SiT 1.72 and S3GM 1.87 --- while \dmfgen{}
carries 0.139 and the deterministic rows almost none (Geo-FNO 0.048, Senseiver
0.068, MLP-RBF 0.043, gappy POD 0.021). The ordering is the reverse of the 3D
result, where the latent decoder truncated the smallest scales and the ambient
point model retained them (Sec.~\ref{sec:spectra}). Ratios above unity are
errors too: small-scale energy in excess of the truth is spurious texture
rather than resolved structure, and the nearest-neighbor interpolant reaches
2.55 in the same band through piecewise-constant blocking. The accuracy leader
carries almost none of that band --- Geo-FNO wins relative $L_2$ with 0.048 of
the true small-scale energy --- so the distortion--perception split of
Sec.~\ref{sec:dp} reappears here as a spectral one. The samplers ran at their own budgets (\dmfgen{} and latent FM at NFE~4,
SiT at 50 steps, S3GM at 200), and a coarse solver budget can bias a sample
smooth, but that is not the explanation here: re-sampling \dmfgen{} on the same
frame, sensors and seed at SiT's 50 steps raises its small-scale energy only
from 0.139 to 0.185 of the truth (0.174 at 16 steps), against SiT's 1.72 at the
same budget, while its large-scale excess grows (1.40 to 1.67) and its
single-sample error worsens (0.71 to 0.78). The 2D small-scale deficit is
architectural, not a coarse-solver artifact.

\begin{figure*}[tbp]
\includegraphics[width=\textwidth]{figures/spectra_kolmogorov.pdf}
\caption{\label{fig:spectra-kolm}Kolmogorov spectral fidelity at 655 sensors
(1\%), held-out frame, one reconstruction per method. (a) Shell-averaged
energy spectra $E(k) = \sum_{\text{shell}} |\hat{\omega}(k)|^2 / 2k^2$ on
integer shells to the Nyquist limit. The field occupies the doubly periodic
box $[0, 2\pi)^2$ and the saved frame is a full period, so no window is
applied here, unlike the Hann-windowed 3D spectra of
Fig.~\ref{fig:spectra}, whose cutouts are non-periodic sub-blocks; the
estimators differ because the boundary conditions differ. The $k^{-3}$ line
is a reference slope, not a fit: the DNS is steeper at this Reynolds number.
(b) Band-integrated energy relative to truth over $k = 1$--$8$ and
$k = 32$--$128$, on a log axis anchored at unity so deficit and spurious
excess both read as error. All generative entries are single posterior
samples; MLP-RBF, a ninth series past the validated eight-color palette,
appears in (b) only. Sampler budgets are each method's own; the NFE-matched control in the
text shows the budget does not explain the ordering.}
\end{figure*}

\begin{figure*}[tbp]
\includegraphics[width=\textwidth]{figures/gallery_kolmogorov_allbaselines_paper.pdf}
\caption{\label{fig:qual-kolm}Kolmogorov $256^2$, canonical held-out frame (val 256).
Vorticity is the only channel and is observed at 655 sensors (1\%; dots on the
truth panel), with the identical draw in every column. Columns and per-panel
numbers as in Fig.~\ref{fig:qual-cyl}. Gappy POD is visibly over-smoothed ---
two-dimensional turbulence is not low-rank --- the generative samples carry
small-scale texture the deterministic rows lack, and the patch tokenizer's
block structure is directly visible in the SiT panel.}
\end{figure*}
...
\begin{figure*}[tbp]
\includegraphics[width=\textwidth]{figures/gallery_jhu_allbaselines_paper.pdf}
\caption{\label{fig:qual-jhu}3D isotropic turbulence, held-out cube (frame 153),
$z$-midplane. Rows are all four channels: $U_x$ and $U_z$ observed at 19{,}531
sensors each (1\%; in-plane sensors shown as circles), $U_y$ and $p$ never
observed; the sensor set is identical in every column. Per-panel numbers are
single-sample relative $L_2$ over the whole cube. The unobserved rows are the
identifiability wall of Sec.~\ref{sec:identifiability}: latent FM, whose decoder
imposes global field structure, is the only method below 0.7 on either
unobserved channel and places the low-pressure core, while interpolation
returns the training mean, gappy POD extrapolates away from the truth, and the
point-token sampler emits incoherent noise. CoNFiLD, S3GM (which diverges at
this scale), DeepONet++ and Gen4Turb are omitted from the gallery.}
\end{figure*}
...
exclude, and per-call setup that a dedicated benchmark amortizes. The marked
rows are the slow samplers, where the sampling term dominates the overhead, so
the comparison the column supports (orders of magnitude between families) is
unaffected.}
\footnotesize
\begin{ruledtabular}
\begin{tabular}{lcccccc}
...
No method leads every column. SiT-point posts the
...
training-free interpolant --- scores 0.174 on $U_x$, ahead of every learned
row except SiT-point (\dmfgen{} 0.176, FNO3D 0.188, latent FM 0.248), so at
1\% density the learned and generative payoffs concentrate where the problem
is ill-posed --- unobserved channels, sparser sensing
(Fig.~\ref{fig:sensors}) --- and in sampling cost (\dmfgen{} reaches its accuracy in
...
mean pooling in its branch; the labeled DeepONet++ arm (structured pooling)
improves $U_x$ from 0.593 to 0.512 and remains behind the point-attention
family.

S3GM's three rows must be read together. At the published guidance
coefficients ($\alpha_{\rm case}{=}0.5$, $\beta{=}0.4$) the sampler diverges
at this scale by five orders of magnitude, each coefficient independently ---
a property of the guidance at $1.95$M points, not a porting defect. Reducing
both coefficients by two orders of magnitude stabilizes the sampler at 0.682
aggregate; a normalized guidance whose coefficients are fitted on the tuning
split reaches 0.628 with observed-channel errors of 0.260 and 0.254. Both are
labeled deviations from the published method, neither can be quoted as
``S3GM'' without qualification, and neither closes the unobserved-channel gap
(0.98--1.02) that no method in the fleet closes.

The top of this table is partly a tie. Applying the paired analysis of
Sec.~\ref{sec:metrics} to the canonical $n{=}50$ JSONs, latent FM's lead is
...
Two limits bound what this table can support. First, only the three rows
above could be compared frame by frame: the SiT-point, Senseiver, Gen4Turb and
DeepONet rows have no canonical per-snapshot payloads in the artifact set, so
their positions are point estimates without a paired interval and we do not
claim any of them is separated from a neighbor. (A fourth CoNFiLD arm, with
strict 2048-point conditioning, is omitted altogether: its files aggregate to
0.941 relative $L_2$ but carry no protocol stamp, so the artifact cannot
confirm that it saw the canonical sensor draw.) Second, the seed error bar
covers the two 2D regimes only; the 3D rows are single training runs, so a 3D
comparison called resolved is resolved by evaluation-frame resampling alone.
...
\subsection{Sensor density: where learning pays}\label{sec:density}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/perf_vs_sensors}
\caption{\label{fig:sensors}Reconstruction error versus the number of sensor
points, in 2D (left, Kolmogorov $256^2$, full learned fleet) and 3D (right,
JHU $125^3$ cross-cube; solid = observed-channel rel-$L_2$, dashed = aggregate
over all four channels). Colored = learned; gray = classical floors. The same
sensor count that leaves a 5.8-cell mean gap in 2D leaves a 10-cell gap in 3D.
In 3D every observed-channel error falls steadily with sensor count while the
aggregate saturates near 0.54, because the unobserved channels do not identify
(Sec.~\ref{sec:identifiability}); gappy POD is flat at every density. In 2D
the fleet fans out (Sec.~\ref{sec:density}): S3GM passes every other method
from 1965 sensors on, Geo-FNO degrades past the canonical density (1.42 at
10\%, marked off-scale), and Senseiver is nearly flat. Learned rows: canonical
protocol, 50 held-out frames, $K$ as in Table~\ref{tab:kolm}, identical sensor
draws across methods at every density; classical curves on the same draws.}
\end{figure*}

The 2D panel of Fig.~\ref{fig:sensors} settles a question the single-density
tables cannot: \emph{over what range of sensor budgets does learning pay for
itself?} Sweeping the Kolmogorov fleet over
$\{65,164,655,1965,6554\}$ sensors --- $0.1\%$ to $10\%$ of the grid, canonical
draws at every density --- bounds that range on both sides for most of the
fleet, and shows that the bound is itself method-dependent. At the sparse end
learning wins clearly: at 65 sensors Geo-FNO reaches 0.778 and \dmfgen{} 0.887
while IDW manages only 0.968 and gappy POD (1.82) and nearest-sensor (1.12) are
both worse than predicting the training mean. At the dense end the fleet
splits. Plain inverse-distance interpolation reaches 0.208 at 6554 sensors and
beats six of the seven learned rows, the best of which are \dmfgen{} at 0.258
and SiT at 0.261; for those six the crossover falls between 1965 and 6554
sensors, so their case for learning lives below a few percent sensing --- the
regime this benchmark targets, and the one uniform-downsampling protocols
leave (Sec.~\ref{sec:leakage}).

The seventh row inverts that picture. S3GM, the guided video-diffusion prior,
is near the bottom of the fleet at 65 sensors (0.939) and at the top from
1965 sensors on: 0.135 there against SiT's 0.310, and 0.041 at 6554, five
times better than interpolation. Its curve is the steepest in the benchmark
($23\times$ over the sweep against $3.4\times$ for \dmfgen{}), consistent
with posterior-sampling guidance whose likelihood term strengthens with the
measurement count until it overrides the prior --- a mechanism we report as
consistent with the data, not established by it. The gain has two prices:
22\,s per field against 0.28 for \dmfgen{} (Table~\ref{tab:kolm}), and a
stated spread that does not shrink with the error (spread--error rises from
1.07 at 1\% to 2.56 at 10\%, coverage$_{90}$ from 0.80 to 0.98), so the most
accurate reconstruction in the sweep is also its most over-dispersed.

The fleet does not share a slope, and the exceptions are diagnostic. Five
rows fall monotonically over the $100\times$ range (S3GM, \dmfgen{}, SiT,
MLP-RBF, Senseiver). Two do not: Geo-FNO is best at the canonical density and
degrades from there --- 0.417 at 655 sensors, 0.463 at 1965 and \emph{1.42}
at 6554, worse than the training mean --- and latent FM turns the same corner
later and more gently ($0.495\!\to\!0.557$ past 1965 sensors). Both are the
rows whose conditioning path resamples the sensors onto a fixed
representation, a grid or a latent code, so sensors added past the point
where that representation saturates add conditioning mismatch rather than
information. A single-density leaderboard cannot see this: at 1\% Geo-FNO is
the best learned method on the regime, and nothing in that number warns that
ten times the instrumentation would make it three times as bad. Senseiver's
near-flatness is the same lesson from the other direction: it extracts at 65
sensors essentially all it will ever extract. \emph{Which} method pays for
learning is itself density-dependent, which is why the sweep is reported as a
first-class result.

\subsection{Unobserved channels: a fleet-wide identifiability
limit}\label{sec:identifiability}

On 3D turbulence, no method reconstructs the unobserved $U_y$ below 0.60
relative $L_2$, and most sit near or above 1.0, worse than the training mean
(Table~\ref{tab:jhu}). The two partial exceptions (latent FM at 0.727,
CoNFiLD-384d at 0.604) are the methods whose decoders impose the strongest
global field structure, and both pay for it elsewhere. This is not a
per-method defect: at 1\% point sensing on two velocity components, the
instantaneous realization of a third component of 3D turbulence is close to
unidentifiable, and only its statistics are recoverable. The cylinder's
never-observed pressure makes the contrast quantitative: where 3D turbulence
compresses every method into the 0.90--1.00 band, the same measurement on the
cylinder spreads them over an order of magnitude --- gappy POD 0.043, Geo-FNO
0.088, Senseiver 0.095, \dmfgen{} 0.128, SiT 0.274, interpolation 1.000 by
construction (Table~\ref{tab:cyl}). Identifiability is a property of the flow
and the sensing geometry, not of a method class, and a benchmark that
reported only 3D would license the wrong conclusion.
...
The identical wrapper is applied to the generative baselines, so the repaired
comparison is two-sided. The repair is post hoc, rescales stated uncertainty
without sharpening the spread floor, and every repaired number carries its
operating point.
...
($\alpha{=}0.95$, $1.046\!\to\!0.996$), which is the behavior a calibration
repair should have and which an inflation-only heuristic would miss.

The cylinder is the counterexample. There the repair does not transfer: \dmfgen{} overshoots ($0.582\!\to\!1.112$) and
...
across the reporting set; when that set is heterogeneous by construction the
fix must be conditioned on the regime variable, which is why we report the
fit rule, the split and the before/after pair for every row. (The cylinder's
canonical frames are all even-indexed, so its tune/test halves are taken by
position in the frame list; both halves draw from both held-out Reynolds
numbers.)
...
One estimator caveat: the conformal fit bins by
...
number would hide that, and a practitioner who needs calibrated intervals should
take the conformal one while a practitioner who needs an error bar on the mean
should take the scalar.

The same script, applied unchanged to the latent flow-matching dumps (same
frames, sensor draws, $K$ and tuning/test split), makes the comparison
two-sided. Latent FM starts less under-covered than \dmfgen{} at sparse sensing
and more at dense: raw 90\% coverage on $U_x$ is 0.553, 0.654 and 0.661 at
0.1\%, 1\% and 10\%, against 0.403, 0.560 and 0.755, so its deficit barely
responds to sensing density above 1\%. The conformal repair brings it to
nominal as well --- 0.897--0.898 on the observed channels at every density ---
and slightly past nominal on the unobserved ones (0.911--0.912 on $U_y$,
0.916--0.924 on $p$), which is the conservative side. The estimator
disagreement measured above separates the two models: after the repair, latent
FM's spread--error ratio on the observed channels is 1.11--1.12 against
1.19--1.57 for \dmfgen{}, so its observed-channel residuals are closer to
Gaussian, while on pressure it is the heavier-tailed of the two (1.33--1.37
against 1.19--1.20). Neither model's intervals are usable as stated; both are
after a split-conformal step fitted on held-out tuning frames.
...
inference memory. The converse is that grid computation is cheaper
...
the dissipation-band ratios of Sec.~\ref{sec:spectra}) are where the
grid-locked methods lose. S3GM is absent from this plane: its
upstream sampler diverges at 3D scale, and its two labeled retuned arms are
...
entirely on the axes the plane omits --- resolution scaling, discretization
freedom and dissipation-range fidelity, measured here, and behavior under
corrupted operators (Table~\ref{tab:operator}).
...
including the strong ones, scores and \emph{looks} worse. We consider this a
central result of the benchmark, because each contribution to the gap is a
protocol choice, and we measure them separately.

\emph{(i) Splits --- a correctness requirement, not the explanation.}
Turbulence frames are strongly autocorrelated (on the 3D data, $r=1.00$ at
unit separation and 0.67 at separation 100), so a random-in-time split places
every evaluation frame next to a training frame, and we expected the leak to
be large. We measured it with the same probe in both regimes: frozen
checkpoints re-scored on frames they were trained on, which bounds from above
what any random-in-time split could return. On 3D turbulence, 50 frames of
the training cubes chosen by the canonical selection rule and sensed by the
canonical seeded draws give latent FM 0.453 aggregate relative $L_2$ against
0.469 on the held-out cube (observed $U_x$ 0.240 against 0.248 and $U_z$
0.277 against 0.257; unobserved $U_y$ 0.665 against 0.727) and FNO3D 0.586
against 0.586, while the training-free interpolation control is unchanged
(IDW 0.589 against 0.589), so the training frames are not easier. \dmfgen{}
is the one row with a material in-sample gain, 0.515 against 0.593
aggregate, and it sits almost entirely on the never-observed pressure (0.678
against 0.964) while its observed channels move by 5\% or less ($U_x$ 0.167
against 0.176, $U_z$ 0.190 against 0.182): memorization buys something only
where the sensors constrain nothing, and even there the channel stays
unrecovered.
On Kolmogorov we re-evaluated frozen checkpoints on frames drawn from
\emph{training} trajectories, two raw frames from a training frame ($r =
0.97$ to the nearest one; raw-frame correlation 0.992 / 0.972 / 0.916 at
lags 1 / 2 / 4): \dmfgen{} $0.487 \rightarrow 0.483$, SiT $0.494 \rightarrow
0.491$, Senseiver $0.913 \rightarrow 0.873$, against an IDW control that
moved $0.546 \rightarrow 0.537$ and therefore sets a ${\approx}1.6\%$
frame-difficulty floor. Net of the controls, no method in either regime gains
more than five percent on an observed channel from having seen its evaluation
targets during training. At ${\sim}6.5$M parameters and $10^5$--$10^6$ points per frame
these models learn the sensing operator rather than a lookup table, and the
protocol gap to the literature must come from elsewhere. (An earlier
evaluation on a single snapshot of a 617-consecutive-frame cutout, made
before the present protocol and its audit repairs, had suggested a factor of
four between shuffled and disjoint splits; it is superseded by the
measurement above.) The split remains a correctness requirement --- a
benchmark whose test frames are training frames measures nothing about
generalization, and the artifact's tooling enforces it --- but its numerical
effect at these data scales is small.
...
sensor \emph{layout} alone, before any change of model or split. The
interpolation floor moves the same way because uniformly spaced sensors bound
the worst-case distance to data, and reconstruction error at 1\% sparsity is
dominated by that distance: a benchmark that adopts uniform sensing because
it is convenient reports a systematically easier problem, and no amount of
split hygiene compensates. Combining the lattice \emph{and} the
seen-trajectory split gives 0.364, indistinguishable from the lattice alone,
so at 2D data scales the protocol gap we can reproduce is sensor geometry
rather than leakage. Our headline claim is consequently narrower than we
first drew it: \emph{protocol, not architecture} remains right, but within
protocol it is the sensing layout that dominates, and the split matters mainly
...
pre-registered frames (on the Kolmogorov gallery frame the four leading
rows score $0.57$--$0.82$ as single samples, against $0.42$--$0.50$ for the
same rows' fifty-frame ensemble-mean averages), because single-sample
...
flow's intrinsic dimension (laminar, periodic regimes) use gappy POD: on the
cylinder it reaches 0.056 aggregate rel-$L_2$ against 0.093 for the best
learned method, recovers the unobserved pressure channel, and costs a
least-squares solve against a rank-20 basis (Sec.~\ref{sec:cylinder}). On observed channels of turbulence at
...
ill-posed: unobserved channels, sparser sensing, occluded sensor sets, and
surface-confined sensing (Secs.~\ref{sec:surface}
and~\ref{sec:identifiability}, Table~\ref{tab:operator}). There, deterministic learners
...
measured dissipation-range floor in 3D, while ambient point-cloud generation
buys 3D small-scale fidelity at higher inference cost --- an ordering that
inverts on Kolmogorov (Sec.~\ref{sec:kolmogorov}) --- with geometry-freedom
beyond a single body as the structural advantage the deferred regimes are
...
geometric generality: the corrupted-operator study (Table~\ref{tab:operator})
uses synthetic noise and occlusion on one 2D regime, the surface-sensing study
(Sec.~\ref{sec:surface}) a single body, and a multiphysics regime with
realistic measurement operators (wildfire LES) and an unstructured,
case-varying geometry regime with surface-only sensing (transonic wing family)
are deferred to a follow-up paper, so claims about geometry-freedom beyond the
cylinder are structural properties of the interfaces, not measured results. All learned rows
...
spanning 2D laminar to 3D chaotic flow, eleven audited learned methods against
four classical floors, proper probabilistic scoring, physics metrics
...
\todo{funding/compute acknowledgments; JHTDB dataset
providers.}
\end{acknowledgments}
