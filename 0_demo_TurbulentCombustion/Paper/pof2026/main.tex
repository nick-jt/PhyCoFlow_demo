...
approaches the POD floor at comparable cost. \todo{learned-fleet rows
(training released after the smoke gate), cross-Re breakdown, surface-tap
variant, ensemble-collapse (spread) check for the generative rows.}
...
classical floors on all 640). Table~\ref{tab:kolm} reports the first
learned rows alongside the floors, and Fig.~\ref{fig:sensors} (left) the
sensor-count sweep. Two regime findings stand out. First, \emph{the 3D
ranking does not transfer}: the ambient point methods \dmfgen{} (0.487
rel-$L_2$, 0.235 CRPS) and SiT (0.494, 0.234) are statistically tied at the
front and both beat inverse-distance weighting (0.546), while latent FM ---
the 3D aggregate winner --- trails the interpolation floor here (0.600) at
the matched ${\sim}50$k-step budget. Second, the floors carry regime
information of their own: gappy POD's rank-80 training basis captures only
70.5\% of the training-set energy --- two-dimensional turbulence is not
low-rank in the POD sense, unlike the cylinder --- and plain
inverse-distance weighting beats it by 20\% relative $L_2$.
\todo{Senseiver/MLP-RBF/GeoFNO/S3GM rows (training/eval in queue); paired
CIs; 2D spectra panel.}
...
all 640). Inference cost per field on one H100.
\todo{remaining fleet rows; ens.\ vs single rel-$L_2$ split; paired CIs.}}
\footnotesize
\begin{ruledtabular}
\begin{tabular}{lccccc}
Method & Rel.\ $L_2$ & CRPS & Cov.$_{90}$ & s/field & GB \\
\hline
\dmfgen{} (NFE 4) & \textbf{0.487} & \textbf{0.235} & 0.59 & 0.19 & 0.76 \\
SiT (50 steps) & 0.494 & 0.234 & 0.64 & 0.18 & 0.24 \\
Latent FM (NFE 4) & 0.600 & 0.310 & 0.64 & \textbf{0.03} & \textbf{0.19} \\
IDW ($k{=}8$, periodic) & 0.531 & 0.375 & --- & --- & --- \\
IDW ($k{=}8$, non-per.) & 0.546 & 0.385 & --- & --- & --- \\
Nearest neighbor (per.) & 0.582 & 0.371 & --- & --- & --- \\
...
where random-in-time splits remain standard. \todo{3-method
shuffled-vs-honest 3D table from the archived pre-fix evaluations.}
