...
\todo{this sentence previously carried \texttt{picsb2026taxonomy}, a
placeholder whose stated identifier (arXiv:2603.22319) was checked and resolves
to an unrelated paper --- ``Sparsely-Supervised Data Assimilation via
Physics-Informed Schr\"odinger Bridge'' --- which proposes no sensor-mask
taxonomy. Removed rather than repointed: the claim is our own design statement
and stands without a citation. If a specific reference was intended, supply it;
do not restore the placeholder.}
...
$L_2$ is beaten on CRPS by every generative model (0.267 against S3GM 0.231, SiT 0.234, \dmfgen{} 0.235), and the split is
resolved on paired snapshots: Geo-FNO trails \dmfgen{} on CRPS by 0.032 (95\%
CI $[0.026, 0.037]$, 46 of 50), while \dmfgen{} and SiT are indistinguishable
...
budget. \todo{CONSISTENCY: this paragraph makes the budget-matched
0.417 the reported row (\texttt{kolmogorov2d\_fullbudget}, 625 epochs = 50{,}000
steps, eval verified at 0.4165), but Table~\ref{tab:kolm}, the abstract, the
density sweep (Fig.~\ref{fig:sensors}) and the operator matrix all use the
$4\times$-budget checkpoint at 0.385. Choose one as canonical and propagate;
the density and operator runs would need re-running if the matched checkpoint
is chosen. Input for the decision, now measured on the matched checkpoint
(\texttt{kolmogorov2d\_fullbudget}): it keeps the lead at the canonical density
(0.417 against \dmfgen{} 0.487); its density curve is non-monotonic more
strongly than the $4\times$ checkpoint's, degrading from 0.417 at 655 sensors to
0.463 at 1965 and 1.421 at 6554; and it costs $1.01$/$1.10$/$1.33\times$ under
noise 0.1 / 0.3 and 25\% occlusion. Every qualitative claim survives either
choice; only the numbers change.}
...
carries 0.139 and the deterministic rows almost none (Geo-FNO 0.058, Senseiver
...
carries almost none of that band --- Geo-FNO wins relative $L_2$ with 0.058 of
...
method, interpolation included, from 1965 sensors on --- Geo-FNO and latent FM
\emph{degrade} past $\sim$2000 sensors, and Senseiver is nearly flat
...
single-density ranking in Table~\ref{tab:kolm}, Geo-FNO, is sixth of seven at
10\% sensing, and the 10\% leader is a row the 1\% table ranks fourth.
...
$0.942\!\to\!0.907$). Two do not: Geo-FNO
...
rows score $0.52$--$0.82$ as single samples, against $0.39$--$0.50$ for the
